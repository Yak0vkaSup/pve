...
class AdvancedBacktestNode(Node):
    def execute(self):
        ...
        try:
            ...
            self.output_values["Backtest Result"] = None
            self.output_values["dca_settings"] = self.dca_settings
        except Exception as e:
...
class SimpleBacktestNode(Node):
    def execute(self):
        ...
        try:
            ...
            # Log and output the backtest result
            logger.info(f"SimpleBacktestNode {self.id}: Backtest skipped.")
            self.output_values["Backtest Result"] = None
        except Exception as e:
...
def process_graph(graph_json, df, symbol):
    ...
    nodes = build_nodes(graph_data['nodes'])
    build_connections(graph_data['links'], nodes)
    ...
    for node_id, node in nodes.items():
        if node.type in ["backtest/simple_backtest", "backtest/advanced_backtest"]:
            settings = node.output_values.get("dca_settings")
            break
